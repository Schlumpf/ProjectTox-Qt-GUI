    ...
    QTextEdit(parent)
{
    ...
    connect(this, &InputTextWidget::cursorPositionChanged, this, &InputTextWidget::resetFormat);
}
...
void InputTextWidget::insertSmiley(const Smiley &smiley)
{
    if (smiley.type() == Smiley::Pixmap) {
        insertHtml(QString("<img src=\"%1\" />").arg(smiley.graphics()));
...
void InputTextWidget::resetFormat()
{
    QTextCursor c(textCursor());
    if (c.anchor() == c.position()) { // no selection
